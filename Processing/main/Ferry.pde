...
class Ferry extends U3DObject{
  ...
  Ferry(Universe uni, int lg){
    ...
    super(uni, new PVector(-14.88295,0,125 + 20.2 + lg*18.1416), "ferry_proue.obj");
    ...
    // Facade arrière
    facadeArriere = new U3DObject(uni, new PVector(-0.11, -1.6294+2.3, -24.9-(lg-1)*16.1113).add(mPosition), "ferry_coqueArriere.obj");
    facadeArriere.disableCollisions();
    
    // Porte RDC
    porteRDC = new U3DObject(uni, new PVector(0.0, 3.35, -24.8-(lg-1)*16.1113).add(mPosition), "ferry_porteRDC.obj");
    porteRDC.setRotationXCenter(new PVector(0, -2.6, 0.69).add(porteRDC.getPosition()));

    // Porte Pont2
    portePont2 = new U3DObject(uni, new PVector(-0.17, 4.1551237+2.3, -24.222954-(lg-1)*16.1113).add(mPosition), "ferry_porte2.obj");
    portePont2.setRotationXCenter(new PVector(-0.5, -0.5, 0).add(portePont2.getPosition()));
    
    // Porte Pont3
    portePont3 = new U3DObject(uni, new PVector(-0.06, 7.53+2.3, -24.182954-(lg-1)*16.1113).add(mPosition), "ferry_porte3.obj");
    portePont3.setRotationXCenter(new PVector(-0.5, -0.5, 0).add(portePont3.getPosition()));
    lvl3Access = false;
    
    while(lg > 0){
      // Coque rez-de-chaussée
      coques.add(new U3DObject(uni ,new PVector(-0.2, -2.8000002+2.3, lg*-16.1113).add(mPosition), "ferry_coque.obj"));
      
      // Coques latérales
      coqueBabord.add(new U3DObject(uni, new PVector(-8.559996, 5.199997+2.3, lg*-16.1416).add(mPosition), "ferry_coqueLaterale.obj"));
      
      U3DObject tmpObj = new U3DObject(uni, new PVector(8.2984, 5.199997+2.3, lg*-16.1416).add(mPosition), "ferry_coqueLaterale.obj");
      tmpObj.setAngles(new PVector(0,PI,0));
      coqueTribord.add(tmpObj);
      
      // Plancher niveau 1
      planchers1.add(new U3DObject(uni, new PVector(-0.2, 3.5999982+2.3, lg*-16.1113).add(mPosition), "ferry_plancher.obj"));
      
      // Plancher niveau 2
      planchers2.add(new U3DObject(uni, new PVector(-0.2, 7.0019982+2.3, lg*-16.1113).add(mPosition), "ferry_plancher.obj"));

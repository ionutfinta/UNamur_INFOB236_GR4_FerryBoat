...
public class fifthRef{
	...
	public Integer integral(BRelation<Integer, Integer> in) {
		if(in.int_size() == 0) return 0;
		else
			return in.apply(in.domain().max()) + integral(in.difference(new BSet<Pair<Integer, Integer>>(new Pair<Integer,Integer>(in.domain().max(),in.apply(in.domain().max())))));
	}

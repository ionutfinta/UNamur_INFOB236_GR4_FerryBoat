...
public class Switch_lvl_1_access extends Thread{
	...
	public /*@ pure */ boolean guard_Switch_lvl_1_access( Boolean boolean) {
		return (BOOL.instance.has(boolean) && !boolean.equals(machine.get_lvl_1_access()) && machine.get_lift_level().equals(new Integer(1)) && BOOL.implication(boolean.equals(true),machine.get_Sensor_state().apply(new Integer(1)).equals(machine.Detecting)));
	}
	...
	/*@ public normal_behavior
	 ...
	 also
		...
		assignable \nothing;
		ensures true; */
	public void run_Switch_lvl_1_access( Boolean boolean){
		if(guard_Switch_lvl_1_access(boolean)) {
			Boolean lvl_1_access_tmp = machine.get_lvl_1_access();

			machine.set_lvl_1_access(boolean);

			System.out.println("Switch_lvl_1_access executed boolean: " + boolean + " ");
		}
	}
	...
	public void run() {
		while(true) {
			...
			run_Switch_lvl_1_access(boolean);

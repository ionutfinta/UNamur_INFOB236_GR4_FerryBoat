...
public class LeaveLift extends Thread{
	...
	public void run_LeaveLift( Integer vehicle_id, Integer vehicle_type){
		if(guard_LeaveLift(vehicle_id,vehicle_type)) {
			...
			machine.set_lift_vehicles(lift_vehicles_tmp.difference(new no_type(new JMLObjectSet {Pair<Integer,Integer> x | (\exists no_type e; (BRelation.cross(new Enumerated(1,machine.queues),new Enumerated(1,machine.lift_depth)).has(null) && lift_vehicles_tmp.apply(null).equals(vehicle_id)); e.equals(new Pair<Integer,ERROR>(null,vehicle_id)))})));
	...
	public void run() {
		while(true) {
			...
			run_LeaveLift(vehicle_id,vehicle_type);

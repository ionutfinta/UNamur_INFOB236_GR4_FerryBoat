...
public class BoardLift extends Thread{
	...
	public /*@ pure */ boolean guard_BoardLift( Integer vehicle_id, Integer vehicle_type, Integer position_q, Integer position_s) {
		return ((new Integer(machine.vehicle_slot.apply(vehicle_type) + machine.get_bs_m())).compareTo(machine.max_bs_m) <= 0 && machine.get_lift_level().equals(new Integer(1)) && machine.get_auth_on_ids().has(vehicle_id) && machine.VEHICLE_TYPES.has(vehicle_type) && new Integer(new JMLObjectSet {Integer fInteger v | (exists Integer e; (machine.floors.has(f) && v.equals(vehicle_type)); e.equals(new Pair<Integer,Integer>(f,v)))}).has(machine.get_reservations().apply(vehicle_id)) && new Enumerated(new Integer(1),machine.queues).has(position_q) && BOOL.implication(machine.get_id_is_left().apply(vehicle_id).equals(true),position_q.equals(new Integer(1))) && BOOL.implication(machine.get_id_is_left().apply(vehicle_id).equals(false),position_q.equals(new Integer(2))) && new Enumerated(new Integer(1),machine.lift_depth).has(position_s) && true && machine.get_lift_access().equals(true) && machine.get_lift_in().equals(true));
	}
	...
	public void run() {
		while(true) {
			...
			run_BoardLift(vehicle_id,vehicle_type,position_q,position_s);
